...
import api from '../../api/insuranceAPI'

const CarInsurance = ({ carId }) => {
  ...
  const getDate = async () => {
    await api.getInsuranceDate(carId).then(res => {
      setDate(res.inspection.end_date)
    })
  }

  const updateDate = async () => {
    await api.updateInsuranceDate(carId, date)
  }

  const deleteDate = async () => {
    try {
      await api.deleteInsuranceDate(carId)
      setDate('none')
    } catch (err) {
      console.log(err)
    }
  ...
  return (
    <div className="w-72 text-center mt-10">
      <p className="font-bold text-xl">Car Insurance Date: </p>
